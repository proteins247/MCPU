...
#  mutate_residue.py
#
#     Usage:   python mutate_residue.py modelname resid resname chain > logfile
#
#     Example: python mutate_residue.py 1t29 1699 LEU A > 1t29.log
#
#
...
#first argument
modelname, resid, resname, chain, = sys.argv[1:]
...
# vz: initiates modeller environment. modeller.environ
...
# vz: modeller.model "holds model of a protein"
mdl1 = model(env, file=modelname)
ali = alignment(env)            # vz: modeller.alignment: sequence alignment
ali.append_model(mdl1, atom_files=modelname, align_codes=modelname)

#set up the mutate residue selection segment
# vz: modeller.selection
s = selection(mdl1.chains[chain].residues[resid])

#perform the mutate residue operation
s.mutate(residue_type=resname)
#get two copies of the sequence.  A modeller trick to get things set up
ali.append_model(mdl1, align_codes=modelname)
# vz: more details from https://salilab.org/modeller/8v0/manual/node125.html
#   adds mutated sequence to "alignment arrays"
...
mdl1.write(file=modelname+resname+resid+'.tmp')
mdl1.read(file=modelname+resname+resid+'.tmp')
...
#give a proper name
mdl1.write(file=modelname+resname+resid+'.pdb')

#delete the temporary file
os.remove(modelname+resname+resid+'.tmp')
